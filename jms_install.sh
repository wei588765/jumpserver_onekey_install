...
# 安装的Jumpserver版本号，这里是最新的1.5.6版本
Version=1.5.6

# Jumpserver 加密配置
# 加密秘钥 生产环境中请修改为随机字符串，请勿外泄, 可使用下面命令生成 
# cat /dev/urandom | tr -dc A-Za-z0-9 | head -c 49;echo
SECRET_KEY=xxx
# 预共享Token，coco和guacamole用来注册服务账号，不在使用原来的注册接受机制，可使用下面命令生成
...
# 如果前面配置的数据库地址为127.0.0.1，则安装mariadb
...
# 如果前面配置的redis地址为127.0.0.1，则安装redis
...
if [ ! -d "$install_dir/jumpserver" ]; then
    ...
    # 官方安装脚本中，相关依赖版本错误，以下为修正软件版本操作
    sed -i "s/Django==2.1.11/Django==2.2/g" $install_dir/jumpserver/requirements/requirements.txt
    sed -i "s/cryptography==2.3.1/cryptography==2.8/g" $install_dir/jumpserver/requirements/requirements.txt
    sed -i "s/pyasn1==0.4.2/pyasn1==0.4.8/g" $install_dir/jumpserver/requirements/requirements.txt
fi
